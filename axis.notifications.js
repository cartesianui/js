...
axis.notifications = axis.notifications || {};
      ...
      axis.notifications.getUserNotificationStateAsString = function (userNotificationState) {
        switch (userNotificationState) {
          ...
          case axis.notifications.userNotificationState.UNREAD:
            return 'UNREAD';
          default:
            axis.log.warn('Unknown user notification state value: ' + userNotificationState)
            return '?';
        }
      };
      ...
      axis.notifications.messageFormatters['Axis.Notifications.MessageNotificationData'] = function (userNotification) {
        return userNotification.notification.data.message || userNotification.notification.data.properties.Message;
      };

      axis.notifications.messageFormatters['Axis.Notifications.LocalizableMessageNotificationData'] =
      ...
      axis.notifications.getFormattedMessageFromUserNotification = function (userNotification) {
        var formatter = axis.notifications.messageFormatters[userNotification.notification.data.type];
        if (!formatter) {
          axis.log.warn('No message formatter defined for given data type: ' + userNotification.notification.data.type)
          return '?';
        }

        if (!axis.utils.isFunction(formatter)) {
          axis.log.warn('Message formatter should be a function! It is invalid for data type: ' +
      ...
      axis.notifications.showUiNotifyForUserNotification = function (userNotification, options) {
        var message = axis.notifications.getFormattedMessageFromUserNotification(userNotification);
        var uiNotifyFunc = axis.notifications.getUiNotifyFuncBySeverity(userNotification.notification.severity);
